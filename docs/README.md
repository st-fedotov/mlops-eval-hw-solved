...
7. **Promote.** When a version's metrics clear your bar, assign the `Production` alias to it: `mlflow models set-alias travel-assistant Production N`. Restart the service with `ASSISTANT_MODEL_ALIAS=Production` in the env, and it loads from the Registry.
