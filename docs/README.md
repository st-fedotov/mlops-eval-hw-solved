...
- An eval CLI (`python -m src.eval`) that runs the whole dataset through the configured variant and logs everything to MLflow.
...
- An MLflow tracking server, Prometheus, and Grafana — all defined in `docker-compose.yml`.
...
5. `python -m src.eval --variant v1` to run the baseline eval. Open MLflow UI at http://localhost:5000 and look at per-category accuracy and cost.
6. **Iterate.** Build new variants by editing `variants.yaml` (different system prompts, different main models, different guardrail configurations). For each variant, run the eval. Each invocation is a new MLflow run.
7. **Watch live monitoring.** With the service running, generate traffic via a script or `curl`; open Grafana at http://localhost:3000 and look at the *Travel Assistant — Live Monitoring* dashboard. The key panel is **DIVERGENCE: cheap refusal-rate vs judge leakage-rate** — when they disagree, you've shipped a regression.
...
| `avg_latency_seconds` | how slow your variant is |
...
- The `variants.yaml` corresponding to your best variant.
...
- A screenshot of the live-monitoring Grafana dashboard taken while your best variant is running and being queried.
...
- The judge has a per-call cost. During development, use `--limit 25`; run the full 100 only before committing.
