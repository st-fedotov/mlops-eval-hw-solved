...
6. **Iterate.** Add new files to `configs/` (different system prompts, different main models, different guardrail configurations). A YAML file in `configs/` is invisible to MLflow until you eval it — local files alone don't reach the tracking server or Registry. For each new config, run `python -m src.eval --config <id>`: each invocation is a new MLflow run; each *full* eval (no `--limit`) is additionally a new registered version, which is the only thing deployable in production mode.
